...
func TestApplyStringWithCustomResource(t *testing.T) {
	...
	// Test with a custom resource (this will likely fail but tests the logic)
	yamlContent := `
...
`
	...
	err := applier.ApplyString(ctx, yamlContent)
	// This will likely fail due to missing CRD, but that's expected
	// The test verifies that the package handles custom resources gracefully
